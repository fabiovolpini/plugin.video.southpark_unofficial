...
class SP_Paths(object):
    ...
    def __init__(self, addon_id):
        ...
        self.PLUGIN_ICON = self.translate_path(
            'special://home/addons/{0}/icon.png'.format(addon_id))
        self.DEFAULT_FANART = self.translate_path(
            'special://home/addons/{0}/fanart.jpg'.format(addon_id))
        self.DEFAULT_IMGDIR = self.translate_path(
            'special://home/addons/{0}/imgs/'.format(addon_id))
        self.TEMPORARY_FOLDER = self.translate_path('special://temp/southpark')
        self.PLUGIN_DATA = self.translate_path(
            'special://temp/southpark/data_{}.json')
...
class SP_Options(object):
    ...
    def debug(self):
        ...
        log_error("OPTIONS Show Subtitles       {0}".format(
            self.show_subtitles()))
...
class SouthParkAddon(object):
    ...
    def __init__(self, argv, addon_id='plugin.video.southpark_unofficial'):
        ...
        self.data = _load_data(self.options.audio(True),
    ...
    def add_directory(self, name, season, mode, iconimage="DefaultFolder.png"):
        ...
        ok = xbmcplugin.addDirectoryItem(
            handle=self.phandle, url=u, listitem=liz, isFolder=True)
        return ok
    ...
    def add_entry(self, name, url, mode, iconimage, desc="", season="", episode="", date="", is_playable=False):
        ...
        url = "{0}?mode={1}&season={2}&episode={3}".format(
            self.argv[0], mode, season, episode)
        convdate = _date(date)
        ...
        entry.setInfo(type="Video", infoLabels={
                      "Title": name, "Plot": desc, "Season": season, "Episode": episode, "Aired": convdate})
    ...
    def add_episode(self, episode):
        ...
        self.add_entry(ep_title, ep_uuid, ep_mode, ep_image,
                       ep_desc, ep_seas, ep_numb, ep_aird, is_playable=True)

    def create_menu(self):
        self.add_entry(self.i18n.MENU_RANDOM_EPISODE, '', PLUGIN_MODE_RANDOM,
                       self.paths.PLUGIN_ICON, is_playable=self.options.playrandom())
    ...
    def create_random(self):
        if not self.options.playrandom():
            self.notify(self.i18n.WARNING_LOADING_RANDOM_EPISODE,
                        WARNING_TIMEOUT_SHORT)
        retries = 0
        while retries < 10:
            retries += 1
            episode_data = self.data.random()
            if len(episode_data['mediagen']) < 1:
                log_error("Found locked episode '{0}'. trying again!".format(
                    episode_data["title"]))
                continue
            if self.options.playrandom():
                self.play_episode(
                    episode_data['season'], episode_data['episode'])
            else:
    ...
    def play_episode(self, season, episode):
        ...
        self.notify("{0} {1}".format(self.i18n.WARNING_LOADING,
                    _encode(data["title"])), WARNING_TIMEOUT_SHORT)
        streams = []
        subtitles = []
        try:
            for url in data["mediagen"]:
                ...
                subs = _dk(
                    mediagen, ["package", "video", "item", 0, "transcript", 0, "typographic"], [])
                subs = list(
                    filter(lambda x: "format" in x and x["format"] == "vtt", subs))
                if len(subs) > 0:
                ...
                try:
                    m3u8 = _dk(
                        mediagen, ["package", "video", "item", 0, "rendition", "src"], None)
                except TypeError:
                    m3u8 = _dk(
                        mediagen, ["package", "video", "item", 0, "rendition", 0, "src"], None)
        ...
        if len(streams) < 1:
            self.notify(self.i18n.WARNING_UNAVAILABLE_EPISODE,
                        WARNING_TIMEOUT_LONG)
            xbmcplugin.endOfDirectory(self.phandle)
            return
        ...
        for i in range(0, parts):
            ...
            title = data["title"]
            if len(streams) > 1:
                title = "{title} ({i}/{n})".format(title=title,
                                                   i=(i + 1), n=parts)

            playitem.setArt({'icon': data["image"], 'thumb': data["image"]})
            playitem.setInfo(
                'video', {'Title': title, 'Plot': data["details"]})
        ...
        if self.phandle == -1:
            ...
            xbmcplugin.setResolvedUrl(
                handle=self.phandle, succeeded=True, listitem=firstitem)
        xbmcplugin.endOfDirectory(self.phandle)
