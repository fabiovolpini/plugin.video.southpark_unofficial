...
def _make_episode(data, season, episode, lang):
    ...
    try:
        args = "uri=mgid:arc:episode:{mediagen}:{uuid}&configtype=edge&ref={dom}{ref}".format(
            mediagen=mediagen, uuid=ep["uuid"], dom=domapi, ref=ep["url"])
        url = "https://media.mtvnservices.com/pmt/e1/access/index.html?{args}".format(
            args=args)
        ...
        urls = []
        for url in items:
            items[i] = _dk(url, ["group", "content"], "").replace(
                "&device={device}", "") + "&format=json&acceptMethods=hls"
            i += 1
        if len(items) > 0:
            urls = items
        ep["mediagen"] = urls
    ...
    print("s{:<2}e{:<2} len:{}: {}".format(
        ep["season"], ep["episode"], len(ep["mediagen"]), ep["title"]))
    i = 0
    for url in ep["mediagen"]:
        ep["mediagen"][i] = base64.b64encode(
...
def _parse_episodes(data, season, lang, inverted, referer_url):
    ...
    if lang in ["en", "es", "eu"]:
        lists = list(
            filter(lambda x: "type" in x and x["type"] == "LineList", lists))
        extra = list(filter(lambda x: _has_extra(x), [
                     _dk(s, ["props"], []) for s in lists]))
        lists = list(filter(lambda x: len(x) > 0 and "url" in x[0], [
                     _dk(s, ["props", "items"], []) for s in lists]))[0]
    elif lang in ["se", "de", "br", "lat"]:
        lists = list(filter(
            lambda x: "type" in x and x["type"] == "LineList" and "type" in x["props"] and x["props"]["type"] == "video-guide", lists))
    ...
    lists = [_make_episode(lists[i], season, n_episodes - i -
                           1 if inverted else i, lang) for i in range(0, len(lists))]

    if len(extra) > 0:
        url = _dk(extra[0], ["loadMore", "url"], "")
        if len(url) > 0:
            extra = _http_get(
                domapi + url.replace(':', '%3A'), True, referer_url)
            if extra != None:
                n_extras = len(extra["items"])
                lists.extend([_make_episode(extra["items"][i], season, (n_extras - i -
                             1 if inverted else i) + n_episodes, lang) for i in range(0, n_extras)])
        else:
...
def _download_data(url, html_links):
    ...
    if "window.__DATA__" in webpage:
        ...
        if html_links:
            links = re.findall(
                r"href=\"/seasons/south-park/[\w]+/[\w]+-\d+", webpage, flags=re.M)
...
def generate_data(lang, old_data):
    ...
    if "links_found" in data:
        ...
        seasons_urls = [_dk(s, ["url"]) for s in _dk(
            main, ["type|SeasonSelector", "props", "items"], [])]
...
def main():
    ...
    parser.add_argument('--only-last-season', action='store_true',
                        default=False, help='updates only the last season')
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument('--en', action='store_true',
                       default=False, help='language english (north america)')
    group.add_argument('--es', action='store_true',
                       default=False, help='language spanish (north america)')
    group.add_argument('--de', action='store_true',
                       default=False, help='language german (germany)')
    group.add_argument('--se', action='store_true',
                       default=False, help='language swedish (sweden)')
    group.add_argument('--eu', action='store_true',
                       default=False, help='language english (europe)')
    group.add_argument('--br', action='store_true',
                       default=False, help='language portuguese (brazil)')
    group.add_argument('--lat', action='store_true',
                       default=False, help='language spanish (latin america)')
